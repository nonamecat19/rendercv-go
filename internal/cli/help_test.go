...
// TestEmbeddedHelpModelParses holds the invariant `loadHelpModel` used to hold
// with a panic.
//
// The decoder now returns the zero model on a malformed embed, so the failure
// has to be caught here instead of in a user's terminal. It is a build problem
// — `helpdata/help.json` is generated by `just helpprobe` — and this is the
// build noticing it: a page with no usage line and no root command is a capture
// that did not decode.
func TestEmbeddedHelpModelParses(t *testing.T) {
	...
	for _, name := range []string{"render", "new", "create-theme"} {
		if _, ok := model.Commands[name]; !ok {
			t.Errorf("the embedded help model has no %q command", name)
		}
	}
}
