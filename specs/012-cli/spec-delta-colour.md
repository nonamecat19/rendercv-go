...
unconditionally (`internal/cli/panel.go:23-30`). Measured on a PTY with `TERM=dumb COLUMNS=100`:
...
One capture rule, learned the hard way and worth repeating: **do not capture into a path containing
`rendercv-go`.** The D-009 binary-name substitution rewrites the capture and corrupts it. Use a
...
(`internal/cli/panel.go:79-120`, `internal/cli/table.go:42-77`).
...
sites without restructuring. Two constraints hold everywhere:

1. **Width arithmetic must keep seeing plain text.** `cellLen`, `columnWidths`, `wrap` and
   `truncate` all measure display cells; if an escape sequence ever reaches them every column
   shifts. Style must be applied *after* layout, at emission.
2. **The decision must be made once, at startup, and passed down** — a package-level global would
   make the table and panel tests environment-dependent. `Panel` and `Table` are pure
   `string`-returning functions today, and that is worth keeping.
...
|---|---|---|
| **A** | PTY differential harness + the §3 env matrix, red against the port on one surface | — |
| **B** | terminal/colour-system detection and a style primitive: §3.1–§3.3 and the §2.2 downgrade table, unit-tested off a fake environment | — |
| C | progress/success panel — `bright_black`, `green`, `bold green`, `purple` (§2.4) | A, B |
| D | error panel + validation table — `bold red`, `cyan`, `magenta`, `orange4`, header `bold` | A, B |
...
**On sizing.** B is the only unit with real subtlety, and its rules are all in §3, measured. C, D, E
are each a small number of write sites once B exists. F is the largest of the leaves because of the
highlighter regexes. A is the one that must not be skipped: without it, every later unit is
self-reported, and self-reported parity is not parity (`AGENTS.md` §10.6).
