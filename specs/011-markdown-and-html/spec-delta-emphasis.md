...
regressions across the HTML differential (641 rows when measured, 761 now) and the whole unit
suite.**
...
`testdata/html.json` and `testdata/markdown_to_typst.json` are machine-generated by construction
(`AGENTS.md` §10.1) but **no tool in this repo generated either of them** — `tools/` had no writer,
and the parent spec's T7 says only "generated through the vendored submodule's
`markdown.markdown`". Every new row was therefore a hand transcription. `tools/mdprobe` (§9, tasks
E-1 and E-2) now owns both, and refuses to write when a committed row does not reproduce.

**Both fixtures reproduce today**: 761 HTML rows and 428 Typst rows, and a full rewrite of each is
byte-identical to what was committed. That retires the trust question for every row added by hand
before the tool existed.

> **Correction.** An earlier revision of this section said the Typst path "has no differential
> fixture at all". It has had one since iteration 8 — `markdown_to_typst.json`, read by
> `TestMarkdownToTypst` (`markdown_test.go:24`), 428 rows, whose own doc comment still says 101.
> What the Typst path lacks is not a fixture but **rows for class E** — and so does the HTML path:
> of the 761 and 428 committed inputs, **zero** put a delimiter beside a non-ASCII space, though
> 172 and 45 respectively contain one elsewhere. That is why the class is invisible to both
> suites, and why §6's prototype moved no row.
...
- the same shapes in `markdown_to_typst.json`, whose 428 rows contain no class-E input today.
  `mdprobe -fixture typst -add` is how they get there.
...
|---|---|---|---|
| E-1 | `tools/mdprobe` generates `html.json` | it reproduces all 761 committed rows byte for byte | no behavior change; unblocks every row below. **Done**, `6824a7f` |
| E-2 | `mdprobe` also owns `markdown_to_typst.json` | it reproduces all 428 committed rows byte for byte | the PDF-visible path's fixture, which existed but had no generator. **Done**, `9658a7d` |
...
- [x] `tools/mdprobe` regenerates `html.json` (761 rows) and `markdown_to_typst.json` (428) and
      both are byte-identical to what was committed; a corrupted row is refused, mutation-checked.
