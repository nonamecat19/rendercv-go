...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 8×, 2026-08-10 — FAIL every pass through the 8th.** Every blocker each pass found is fixed (`396b982`, `31fc0fe`, `852d483`, `d02d593`, `735a17a`, `29cb863`, `a8fb580`, `4caa576`, `221e4cd`, `cdb9447`, `e39cc7f`, `cb62853`, `49210a1`, `facf4dc`, `bfd5990`, `277d85b`). Pass 8 found the pass-7 `font_family`-as-mapping carve-out only checked the mapping's *shape*, not its contents, on both sides that grant it: a script (`ValidateScript`) or a document (`withoutTreeConflictsAt`) declaring `typography.font_family.body` as a nested table one level deeper than the carve-out looked was waved through unchecked — fixed by recursing into `FontFamily`'s own fields on both paths. Pass 8 also found `ValidateScript` never checked list shapes at all — a script declaring a Lua sequence for a scalar field (`page.size = {"a4"}`) merged straight through and printed a Go type name, the same failure mode the map-shape check already caught, just never extended to `KindStringList` — fixed by adding a `shapeKind`-based list check symmetric with the existing map one. All three differentially confirmed against upstream and against the pre-fix binary. **New, undeclared, cut forward**: a custom theme *with* a script still skips the design tree's value validation entirely (`design.Validate` returns after `ValidateTheme` for any non-built-in theme), where upstream runs `theme_data_model_class(**design)` and refuses a bad value on a *known* key — `page.size: bogus` against a scripted theme is upstream exit 1, port exit 0 emitting the bogus value into the artifact. Broader than the already-cut forbid-extra-unknown-keys gap and not yet in `divergences.md`. `themeScript` still discards a broken script's error and falls back silently, exit 0 — the open human-gated question. The 17-vs-8 sandboxed-globals test-coverage gap is still open. Still not green | 0 blockers reproduced by the 8th pass's own fixes (unverified by a 9th), 2 known gaps cut forward, 1 coverage gap open, majors/minors recorded |
