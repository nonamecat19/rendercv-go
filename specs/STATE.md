...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 11×, 2026-08-10 — FAIL every pass through the 11th.** Every blocker each pass found is fixed (`396b982`, `31fc0fe`, `852d483`, `d02d593`, `735a17a`, `29cb863`, `a8fb580`, `4caa576`, `221e4cd`, `cdb9447`, `e39cc7f`, `cb62853`, `49210a1`, `facf4dc`, `bfd5990`, `277d85b`, `4dd2f55`, `96d495a`, `0da9351`, `7256c82`). Pass 11 confirmed pass 10's fix correct (four extra probes beyond its shipped test, all byte-identical) and found a different, broader defect in the same validate/merge pipeline: `validateModel`'s null-skip discarded **every** null-valued design field regardless of kind, and the binder's own `isNull && !Required` skip did the same, because every design field is non-`Required` (all have defaults) which the binder conflated with "admits null" — correct for entry fields (whose non-required strings really are `Optional`) but wrong for the design tree, where only `templates.education_entry.degree_column` is actually `str | None`. `colors.name: null` on `theme: sb2nov` rendered at exit 0 with `rgb(0, 79, 144)` (the *base* tree's color) instead of sb2nov's own `rgb(0, 0, 0)` — reachable on a **built-in** theme, no script involved, no possible byte-diff since upstream produces no artifact at all (exit 1). Fixed with `binder.Field.TypeRejectsNull`, opt-in and zero-risk to the other seven callers (default `false`), set for every design field but `degree_column`; `validateModel`'s local loop now lets a null reach `validateField`, whose per-kind arms (`validColorNode`, `validBoolNode`, the literal and nested-model arms) already had the right behavior and simply never saw one (`7256c82`), landed with 6 new table-driven cases across five field kinds plus the degree_column control. Pass 11 re-confirmed the three pass-10 process findings unchanged (no `plan.md`/`tasks.md`, commit subjects over 50 chars, pre-existing `typstc` lint debt) — still not acted on. Cut forward, unchanged: a custom theme *with* a script skips the design tree's value validation entirely, `themeScript` discards a broken script's error silently (exit 0), and the 17-vs-8 sandboxed-globals coverage gap. Still not green | 0 blockers reproduced by the 11th pass's own fix (unverified by a 12th), 2 known gaps cut forward, 1 coverage gap open, 3 process findings open, majors/minors recorded |
