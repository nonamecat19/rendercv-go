...
| # | Finding | Status |
|---|---|---|
| 1 | **`phrases`, `month_abbreviations` and `month_names` are declared with no value type** (`catalog.go:31-33`; the binder documents the zero value as "never checks"), and `phrases` has no nested spec — so four documents upstream rejects render at exit 0: `phrases: hello`, `phrases: {bogus: x}`, `month_names: hello`, and a list of twelve integers. Axis 4 is broken for every locale member except the six plain strings. | **open** |
