...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 6×, 2026-08-10 — FAIL every pass through the 5th, fixed and re-verified for the 6th.** Every blocker each pass found is fixed (`396b982`, `31fc0fe`, `852d483`, `d02d593`, `735a17a`, `29cb863`, `a8fb580`, `4caa576`, `221e4cd`, `cdb9447`, `e39cc7f`). The 5th pass found a partial `font_family` mapping override dropping its four sibling elements to empty (not the theme's own font, and not the intended base default) and a word-form YAML boolean (`show_footer: no`) being pruned as a type conflict or reaching Typst unconverted — both fixed, and both measured to be wider than the scripted-theme path: they also broke **built-in** themes. The fix for the second introduced a 6th-pass regression of its own — `kindOf` classified any bool-word string as kind "true or false" symmetrically, so a script-declared **string** option against a document override that happened to spell a bool word (`custom_note: 'On'`) was pruned as if it agreed — fixed by gating the exemption on the script's declared value actually being a Go `bool`. The forbid-extra rejection on a scripted custom theme's unknown keys is still cut to a future pass — needs the script loaded during validation, not render. `specs/014-lua-custom-themes/spec.md` §2 behavior 9 and §5 no longer claim the port surfaces a broken script's error text; it doesn't — `themeScript` discards the error and falls back silently, exit 0, which is still the open human-gated question. The 17-vs-8 sandboxed-globals test-coverage gap (6th pass, major) is still open. Still not green | 0 blockers reproduced by the 6th pass's own fixes (unverified by a 7th), 1 known gap cut forward, 1 coverage gap open, majors/minors recorded |
