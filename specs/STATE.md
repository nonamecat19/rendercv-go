...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 12×, 2026-08-10 — FAIL every pass through the 12th.** Every blocker each pass found is fixed (`396b982`, `31fc0fe`, `852d483`, `d02d593`, `735a17a`, `29cb863`, `a8fb580`, `4caa576`, `221e4cd`, `cdb9447`, `e39cc7f`, `cb62853`, `49210a1`, `facf4dc`, `bfd5990`, `277d85b`, `4dd2f55`, `96d495a`, `0da9351`, `7256c82`, `ebfcc30`, `035048e`). Pass 12 confirmed pass 11's null fix unbreakable (adjacent nested/font_family/string-list inputs, all matching) and found four new blockers, none of them regressions and none custom-theme-specific — all reachable on a **built-in** theme: (1) `ValidateTheme` still skipped a null outright rather than stringifying it to `"None"`; (2) `theme: true` used the raw token `"true"`, which passes the lowercase pattern, instead of Python's `str(True)` = `"True"`, which doesn't; (3) a sequence or mapping theme value produced an empty backtick-quoted name in the message instead of Python's own repr; (4) `validColorNode`'s sequence arm checked only tuple length, so `[300, 0, 0]` and `[a, b, c]` validated where the string form `rgb(300,0,0)` already rejects the equivalent, and a *valid* tuple that passed reached the Typst emitter as `bridge.mappingOf`'s raw `[]string` and printed a Go value instead of `rgb(1, 2, 3)` — `normalizeColors` only ever looked for a `string`. Fixed with `themeNameRepr`/`pythonElemRepr` (Python's `str()`/`repr()` of a YAML node, `ebfcc30`) and `ParseColorTuple` (`parse_tuple`'s element-wise range/type check, called from both `validColorNode` and a new `normalizeColors` `[]string` arm, `035048e`), each landed with tests — 5 non-string theme cases, 7 tuple-validation cases, one full-pipeline out-of-range case, one full-pipeline valid-tuple-renders-as-rgb case. Pass 12 re-confirmed the pass-10/11 process findings unchanged (no `plan.md`/`tasks.md`, commit subjects over 50 chars — three passes running, pre-existing `typstc` lint debt) and re-confirmed the "value validation skipped when a script exists" cut-forward gap with a broader repro (`page: null`, `page.size: null`, `colors.name: null`, `typography.font_family: null` all render at exit 0 on a **scripted** custom theme, matching upstream only when the same theme has no script). Cut forward, unchanged: `themeScript` discards a broken script's error silently (exit 0), and the 17-vs-8 sandboxed-globals coverage gap. Still not green | 0 blockers reproduced by the 12th pass's own fixes (unverified by a 13th), 2 known gaps cut forward, 1 coverage gap open, 3 process findings open, majors/minors recorded |
