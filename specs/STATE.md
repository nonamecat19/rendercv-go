...
- **Left open, not blockers:** the AssertPDF/AssertPNG oracle checks only text/pages/geometry even
  though the port achieves byte-identity — a real regression in a theme's colour or an image would
  pass green today; a typst-compile-failure path prints only an error panel on stdout where upstream
  also emits a partial progress panel plus a stderr traceback (same exit code, D-014's class, just
  an untested shape); `9d26f8f` bundled two `tasks.md` units (T6, T7) into one commit, against
  AGENTS.md §7 — historical, not re-split. None of these gate `green`.
