...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 13×, 2026-08-10 — FAIL every pass through the 13th.** Pass 13 found 7 findings (6 blockers, 1 minor); 5 blockers fixed (`5dc89ac`, `20c8e6d`): a colour tuple's bool/hex/octal/binary elements (`parse_color_value` is `float(value)`, which `strconv.ParseFloat` alone rejects), a null alpha (`parse_float_alpha(None)` returns `None`, not an error), a script's own colour tuple (dropped twice over — `ValidateScript`'s shape check, and `luatheme.sequenceOf` silently discarding every non-string element before the tuple ever reached that check), and a document tuple overriding a scripted theme's colour default. **Two findings deferred, not fixed**: (1) `themeNameRepr` uses `node.Raw` for a numeric theme name instead of Python's `str()` of the *parsed* number, so `theme: 1_000` or `theme: 0x1f` — the token, not its decimal value — reaches the pattern check, diverging only on exotic numeric literal forms as a theme name; (7, minor) `pythonElemRepr`'s mapping arm always single-quotes a key, where Python's `repr` of a non-string key is unquoted. Both are edge cases with no plausible CV reaching them (a numeral or numeric-literal theme name); recorded here rather than expanded into a full YAML-number-to-Python-repr subsystem for iteration 14's scope. Pass 13 confirmed pass 12's two fixes hold on 15+ adjacent probes (int/float theme names, empty sequences, nested mappings, standard-decimal tuples, alpha edge cases) and re-confirmed the process findings and cut-forward gaps unchanged. | 0 blockers reproduced by the 13th pass's fixes (unverified by a 14th), 2 deferred findings (1 minor), 2 known gaps cut forward, 1 coverage gap open, 3 process findings open |
