...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 10×, 2026-08-10 — FAIL every pass through the 10th.** Every blocker each pass found is fixed (`396b982`, `31fc0fe`, `852d483`, `d02d593`, `735a17a`, `29cb863`, `a8fb580`, `4caa576`, `221e4cd`, `cdb9447`, `e39cc7f`, `cb62853`, `49210a1`, `facf4dc`, `bfd5990`, `277d85b`, `4dd2f55`, `96d495a`, `0da9351`). Pass 10 found the same empty-table ambiguity one layer deeper than pass 9 reached: pass 9's `ValidateScript` fix let a script's empty table at `sections.show_time_spans_in` through validation, but `EffectiveWithScript` was still `deepMerge`-ing that raw empty Go map over the tree's `[]string` base default — so `withoutTreeConflictsAt` then saw a **map** at that path and dropped a document's real `show_time_spans_in` list override as a shape conflict, silently rendering the wrong CV (byte diff confirmed in `.typ`, `.md` and `.html`) at exit 0. Fixed by normalizing the script's empty map to `[]string{}` before the merge, gated on the tree's `KindStringList` kind the same way `ValidateScript` is (`0da9351`), landed with a regression test asserting the document's override survives end-to-end. Pass 10 also surfaced three process findings, none fixed yet: `specs/014-lua-custom-themes/` has only `spec.md`, no `plan.md`/`tasks.md` (CLAUDE.md §4 requires both before implementation); every one of the 23 iteration-14 commit subjects exceeds AGENTS.md §7's 50-char limit (51-82 chars); `just check` is red on pre-existing `errcheck`/`errorlint` findings in `internal/renderer/typstc` from before this iteration (`0897fcc`/`9b3f3b1`), unrelated to iteration 14's scope but noted as a gate that does not currently pass. Cut forward, unchanged: a custom theme *with* a script skips the design tree's value validation entirely, `themeScript` discards a broken script's error silently (exit 0), and the 17-vs-8 sandboxed-globals coverage gap. Still not green | 0 blockers reproduced by the 10th pass's own fix (unverified by an 11th), 2 known gaps cut forward, 1 coverage gap open, 3 process findings open, majors/minors recorded |
