...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 9×, 2026-08-10 — FAIL every pass through the 9th.** Every blocker each pass found is fixed (`396b982`, `31fc0fe`, `852d483`, `d02d593`, `735a17a`, `29cb863`, `a8fb580`, `4caa576`, `221e4cd`, `cdb9447`, `e39cc7f`, `cb62853`, `49210a1`, `facf4dc`, `bfd5990`, `277d85b`, `4dd2f55`, `96d495a`). Pass 9 found two more holes in pass 8's font_family/list work, both in the fields that had *just* been touched: `withoutTreeConflictsAt`'s font_family carve-out handled the map and scalar cases but let a **list** through unchecked, reaching pongo2 as a slice and surfacing a raw template-engine error instead of a panel (fixed, `4dd2f55`); and `ValidateScript` flagged an *empty* Lua table for the tree's one `KindStringList` field as a conflict, because Lua's single table type makes an empty sequence and an empty mapping the same value and `luatheme.Options` converts a length-0 table to an empty Go map — `themeScript`'s all-or-nothing drop then lost every other option the script declared over one it never meant to touch, the same class of collateral damage as pass 6's finding (fixed by checking `KindStringList` before the generic map check, `96d495a`). Pass 9 also flagged that none of pass 8's three fixes shipped with a test — both pass-9 fixes landed with one, plus regression coverage for pass 8's two. All differentially confirmed against upstream and the pre-fix binary. Cut forward, unchanged: a custom theme *with* a script skips the design tree's value validation entirely, `themeScript` discards a broken script's error silently (exit 0), and the 17-vs-8 sandboxed-globals coverage gap. `just check`'s 2 lint issues predate iteration 14 (`0897fcc`) and are left alone — out of this iteration's scope. Still not green | 0 blockers reproduced by the 9th pass's own fixes (unverified by a 10th), 2 known gaps cut forward, 1 coverage gap open, majors/minors recorded |
