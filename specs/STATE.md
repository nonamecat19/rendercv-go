...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 7×, 2026-08-10 — FAIL every pass through the 7th.** Every blocker each pass found is fixed (`396b982`, `31fc0fe`, `852d483`, `d02d593`, `735a17a`, `29cb863`, `a8fb580`, `4caa576`, `221e4cd`, `cdb9447`, `e39cc7f`, `cb62853`, `49210a1`). Passes 5-7 found the port's boolean and font_family handling was wrong on **built-in** themes too, not just scripted custom ones, and that a fix in one pass could regress a case the same pass's own tests didn't cover: pass 6 found pass 5's bool-word exemption over-widened and dropped a script-declared *string* option; pass 7 found `mappingOf` recognized only a hand-picked subset of the bool spellings `ResolveScalar` itself produces (`TRUE` landed on the wrong boolean, not merely uncoerced) and that `ValidateScript` flagged a script declaring `font_family` as a mapping as a conflict, discarding the *entire* script — every other option it declared, not just that field. **New, undeclared, cut forward**: pass 7 found a custom theme *with* a script skips the design tree's value validation entirely (`design.Validate` returns after `ValidateTheme` for any non-built-in theme), where upstream runs `theme_data_model_class(**design)` and refuses a bad value on a *known* key (not just an unknown one) — `page.size: bogus` against a scripted theme is upstream exit 1, port exit 0 emitting the bogus value into the artifact. This is broader than the already-cut forbid-extra-unknown-keys gap and is not yet in `divergences.md`. The forbid-extra rejection itself is still cut to a future pass — needs the script loaded during validation, not render. `specs/014-lua-custom-themes/spec.md` §2 behavior 9 and §5 no longer claim the port surfaces a broken script's error text; it doesn't — `themeScript` discards the error and falls back silently, exit 0, which is still the open human-gated question. The 17-vs-8 sandboxed-globals test-coverage gap is still open. Still not green | 0 blockers reproduced by the 7th pass's own fixes (unverified by an 8th), 2 known gaps cut forward (one new), 1 coverage gap open, majors/minors recorded |
