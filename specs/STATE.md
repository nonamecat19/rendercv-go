...
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **re-verified 14×, 2026-08-10 — FAIL every pass through the 14th.** Pass 14 found 5 findings (4 blockers, 1 major), all fixed (`bc635d0`, `df4a82a`, `7c2227d`): `parseAlpha`'s non-percent branch still called `strconv.ParseFloat` directly rather than pass 13's `parseNumericText`, so a bool/hex/octal/binary **alpha** was rejected where the equivalent channel already worked (also added `strings.TrimSpace`, matching `float(" 1 ")`); pass 13's `KindColor` carve-out in `validateScript` continued on *any* list shape with no check, so an invalid script tuple (`{300, 2, 3}`) survived validation and printed a raw `[]string` into the artifact — now runs `ParseColorTuple` and reports a `ScriptConflict` on failure; the two colour-tuple-vs-scalar merge carve-outs were each only checked in one direction — `withoutConflicts` (`luatheme.Validate`'s comparison, which cannot import this package's colour parser to know a shape is legal) had **no** colour exemption at all, so a document scalar overriding a script's tuple was dropped, and `withoutTreeConflictsAt`'s carve-out only handled "document is a list, tree is a colour string", not the reverse — both fixed with a shared `looksLikeColorConflict` proxy and a second, mirrored branch. The major: `schemaerr.RenderInput` special-cased `null`→`"None"` but fell through to the raw token for a bool, showing lowercase `true` where pydantic's Input Value column carries `str(True)` — reachable on any design field, a broader site than the numeric-literal case pass 13 deferred, and it shows the deferral's "no plausible CV reaches it" reasoning didn't hold for this sibling site. **Still deferred, unchanged**: `themeNameRepr`'s and `RenderInput`'s shared gap for a numeric field's literal *form* (`0x1f`→`31`, `1_000`→`1000`) — the parsed-number case, not the bool case now fixed — and `pythonElemRepr`'s always-quoted mapping key. Pass 14 confirmed pass 13's null-alpha and `sequenceOf` fixes correct on 15+ adjacent probes. | 0 blockers reproduced by the 14th pass's fixes (unverified by a 15th), 2 deferred findings (1 minor), 2 known gaps cut forward, 1 coverage gap open, 3 process findings open |
