...
## Measured state, 2026-08-14 — pass 26 (iteration 10's first fresh-context verify)

Supersedes pass 25 for iteration 10 only; nothing else was touched this pass. Iteration 10 (row 10)
had been "gate cleared, landed and running in the suite" since 2026-08-08 and never audited by a
fresh context — every other subsystem had been by pass 25. `rendercv-parity-verifier` found **8
findings, 2 blockers**, closed same day:

- **Blocker, fixed:** `typstc.Compile` refused any `OutputPath` outside the input's own directory.
  Upstream has no such restriction — `-pdf out2/cv.pdf`, an absolute path, `../`, and
  `settings.render_command.{pdf,png}_path` pointing elsewhere all render fine upstream and aborted
  the whole render here (no PDF, no PNG, and no `.md`/`.html` either, since the pipeline aborts
  mid-render). Fixed by mounting the output's directory as a second guest root, `/out`, only when
  it is not already under `/work` (`6e341e5`). The old pinning test asserted the bug as correct
  behavior and is replaced with two tests for the real contract: output may live elsewhere, but its
  directory must already exist (matching `path_resolver.py:109`'s `mkdir`, done host-side before
  `Compile` is ever called).
- **Blocker, declared rather than fixed:** the WASI sandbox cannot see the host's installed fonts;
  upstream's `typst-py` can. Two reachable consequences with no unusual config: an unconstrained
  `design.typography.font_family` override falls back differently (measured: a host-installed
  family upstream has access to renders 3 pages there and 2 here), and **any glyph the vendored set
  does not cover — emoji is the practical case — diverges from an otherwise default document**.
  D-007 previously framed host-independence as pure upside and got the font-tie-order winner
  backwards; both are corrected, and the gap is recorded rather than fixed, because giving the guest
  access to an arbitrary host font tree is the sandboxing D-006/D-007 exist to avoid (`471cf5c`).
- **Coverage hole, closed:** no corpus case exercised `cv.photo`. `photo_next_to_typst` is added,
  the photo renders and copies next to the `.typ` identically to upstream, byte for byte
  (`d7e3aab`, `ba9ddde`). The suite's PDF/PNG count moves from 18 to 19.
- **What the verifier confirmed clean, more strongly than the ledger claimed:** 26 hand-run
  differentials (17 font families, photo, CJK±emoji, A4, `fonts/`-directory name ties, a 316-page
  document) found the port's PDFs and PNGs **byte-identical** to the vendored Python's — not just
  matching on text/pages/geometry, which is all axis 1 requires. Font provisioning is exactly
  right: all 77 vendored files are sha256-identical to `rendercv_fonts`, and so is the `fontawesome`
  package and the `rendercv` typst package.
- **The four remaining minors, closed same day:**
  - **The stdout shape on a Typst compiler failure is fixed, not just tested.** `typst.TypstError`
    is neither `RenderCVUserError` nor `OSError`, so upstream's `except` chain never catches it —
    it propagates past `with ProgressPanel(...)` unhandled and `Live` leaves its last frame, the
    progress panel with whatever steps had completed, on stdout. The port called `failPanel`
    unconditionally and showed the red `Error` box instead. `failStep` now recognizes `*typstc.Error`
    specifically and re-renders the accumulated progress panel; every other generate-step error (an
    `OS Error` from a mkdir or a write) still gets the box, matching `except OSError`. A new
    `err_typst_compile_failure` corpus case (a corrupt photo forces the compiler to fail) measures
    stdout **byte-identical**; only stderr — the traceback — still differs, D-014's own class, so
    the case is registered under it in `internal/conformance/unreachable.go` rather than left red
    for an unstated reason (`7a8131b`, `f5e1fe4`, `24a3b15`).
  - **`AssertPDF` now checks embedded fonts, closing a real spec gap, not a stale comment.**
    `specs/000-parity-contract/spec.md` §1.2 already named "embedded font names, as a set" as one of
    the four things a PDF must match; the oracle only ever checked page count, text and geometry.
    `pdffonts()` strips typst's own six-letter subset tag (`XXXXXX+`) before comparing — confirmed
    unstable even between two runs of the port's own binary over the same input, so leaving it in
    would fail on subsetting rather than on a font difference. Ran clean against all 19 PDF corpus
    cases (`5550905`).
  - **`typstc.Compile`'s `ctx` parameter is no longer decorative.** The shared wazero runtime ran a
    module to completion regardless of the caller's context; `WithCloseOnContextDone(true)` makes a
    canceled or timed-out context actually abort that call, closing only its own module instance.
    Pinned by `TestCompileHonorsContextCancellation` (`d37dea5`).
  - **`9d26f8f` bundling T6 and T7 into one commit stays as recorded history.** Rewriting a
    published commit range to split it after the fact is a git-history rewrite this port's own
    safety rules do not do casually, and the two units are already both correct and both tested —
    there is no live defect left to close, only a process note. Left as is, named here so it is not
    re-discovered as new.

---

## Measured state, 2026-08-14 — pass 25 (HEAD after the merges below)

Supersedes pass 24. **29 commits landed** from nine porters in parallel worktrees; every number here
was measured by the merge owner in the main checkout after merging, never taken from a porter's
report. Two porters were resumed after the harness dropped them mid-run.

| Check | Result |
|---|---|
| `go build ./...` | pass |
| `go test ./...` | pass, 0 FAIL |
| `go test -tags conformance ./...` | pass — 0 FAIL, 0 SKIP, 55 corpus cases |
| `just check` | 0 issues |
| `just schema-diff` | empty |
| 9 themes × `.typ`/`.md`/`.html` vs the vendored Python | **byte-identical, all 9** |
| 22 locales × `.typ`/`.md`/`.html` | **byte-identical, all 22** |
| 14 entry/date shapes × **9 themes** | **byte-identical, 126/126** |
| 18 CV-name shapes → generated filenames | identical, 18/18 (including `A/B`, `..`, unicode, tab) |
| 80 `cv` + 80 `locale` + 144 `design` type-mismatch vectors | identical, 304/304 |
| ~770 fuzzer-mutated documents, 5 seeds | exit-code parity on all but the two known classes |
...
Pass 24 measured every check above as green and the ledger read as if only cosmetics remained. It
was wrong, and not because a check regressed — because **nothing in the suite drives documents the
corpus does not contain**. Five differential sweeps written this pass (malformed YAML, CV field
shapes, the CLI surface, settings/overrides, markdown constructs in a real CV field) found:

1. **A panic.** A document with **no `cv:` key at all** — `locale: {language: english}` — exited 2
   with `invalid memory address or nil pointer dereference`. Upstream renders it at exit 0, to
   `NAME_IN_SNAKE_CASE_CV.typ`. The nil was deliberate (`rendercvmodel.go:112`) and contradicted
   this port's own spec 002 §3.28, which already said `cv` defaults. Fixed, `.typ` byte-identical,
   two corpus cases added (`695aa28`, `461bace`).
2. **C0 control characters were accepted.** `cv:\n  name: \x01A\n` rendered a complete CV at exit
   0; upstream exits 1 with `unacceptable character #x0001: special characters are not allowed.`
   Seven of thirteen probed codepoints rendered where upstream refused. ruamel checks the whole
   document when `Reader.stream` is assigned, *before* the scanner — so the new check runs ahead of
   the `%YAML` and tab rules, verified on documents that trip both at once (`e76a457`, `696de14`).
3. **`design:` with no `theme:` rendered at exit 0** where upstream raises `KeyError: 'theme'` and
   exits 1. Four document shapes affected. The scoped decision this row waited on was made: exit-code
   parity is binding and achievable, the traceback text is D-011's declared class, so the port exits 1
   with its own panel. Recorded as D-021 (`7f1ab2f`, `3ae871d`).
4. **`cv.website: []` rendered at exit 0** where upstream exits 1 — D-012's table had this as its one
   "port renders and upstream does not" row and it is now closed (`61e20c9`, `bd8df78`).
5. **The port emitted a validation row upstream suppresses.** A field-level failure on an entry gates
   pydantic's `mode="after"` model validator entirely, so the start-after-end row never runs; the port
   printed it anyway in 7 of 14 measured combinations (`b1360b3`, `5d64c57`).

Two more axis-4 defects fell out of the same sweeps: the Input Value column printed `...` where
upstream prints the value (`None`, `hello`, `5`, `True`, a 140-char string — nine shapes wrong,
`8a0ad0b`), and unterminated character references like `&#35` were escaped rather than terminated
(`a49a343`).

### THE LARGEST OPEN ITEM FOUND THIS PASS — the `settings` tree is essentially unvalidated

Found by a porter told to sweep the neighbourhood of one field rather than fix only that field.
88 type-mismatch vectors through the `settings` tree: **45 disagreed with upstream**, all the same
shape — upstream exits 1, the port exits 0 and renders a complete CV. One mechanism is fixed
(`bold_keywords`, `cba5b8e`, 19 shapes now byte-identical); **41 vectors across five mechanisms
remain open**:

| # | Mechanism | Vectors | Upstream | Port |
|---|---|---|---|---|
| B | `pdf_title: str` given a list/map/int/null | 4 | exit 1, `Input should be a valid string.` | exit 0 |
| C | the six path fields given a list/map/int | 18 | exit 1, `Input is not a valid path for <class 'pathlib.Path'>.` | exit 0 |
| D | `dont_generate_{markdown,html,typst}` given a list/map | 6 | exit 1, `Input should be a valid boolean.` | exit 0 |
| E | `current_date: null` | 1 | exit 1 | exit 0 (`settings.go:39-41` returns nil for `KindNull`) |
| F | `settings`/`render_command` as a scalar/list/null; `render_command.design`/`.locale` as a list/map | 12 | exit 1 **via an uncaught traceback** | exit 0 |

The porter **stopped rather than improvising**, correctly: `specs/006-design-and-themes/settings.md`
§4.1 assigns `render_command`'s behavior to iteration 12 and §5 lists it open, so AGENTS.md §4's
spec-first rule binds. A spec addendum is being written now
(`spec-delta-settings-validation.md`), after which B–E fan out in parallel and F needs the
D-011 precedent decision.

Two measured facts the spec has to preserve, both counter-intuitive: `dont_generate_pdf` and
`dont_generate_png` are **exit 0 upstream even for a mapping**, because the `-nopdf`/`-nopng` flags
...
exit-code disagreements, reduced to minimal documents. One is a defect no type sweep would ever
have reached, because it is about **order**:
...
### Four ledger rows were stale — the code was right and the ledger was not

Measured, not assumed, and each retired here:

- **Iteration 1's "cannot be closed without the human gate."** The fixed-length, checkout-independent
  work root already exists (`internal/conformance/workroot`, `/tmp/rendercv-go-conformance`), with a
  per-case `flock` closing the `caseWorkDir` race and a `checkCanonical` guard. Verified passing from
  both the main checkout **and** an agent worktree — the cross-checkout run the prior audit never did.
- **Iteration 12's YAML span blocker.** The document the row cites now produces
  `main_yaml_file: line 4 to line 6` / `while parsing a block mapping.` — **byte-identical to
  upstream**, both streams.
- **Iteration 12's "no ANSI colour at all on a tty."** False. On a pty the port emits 84 escape
  sequences on the validation panel and its final screen is **identical to upstream's** once the
  nondeterministic timing field is masked. `TestProgressPanelColour` already compares this
  byte-for-byte across seven terminal configurations. The one real stream difference is Rich's
  intermediate `Live` frames, whose count is wall-clock dependent and therefore not byte-comparable
  — the final screen is what a reader sees, and it agrees.
- **Iteration 15's `pythonElemRepr` unit.** Closed by `5228154` two commits before the row was
  written; the function no longer exists.

**The lesson worth keeping:** every one of the four was a row asserting a *conclusion* that no test
pinned, and every one survived multiple passes. The five new defects were all found by driving
documents nobody had driven, not by reading code. Sweeps beat audits here, decisively.

### `just check` was failing on other checkouts' files

`gofumpt -l .` walked `.claude/worktrees/`, so `just check` reported files belonging to 28 agent
worktrees. Both recipes are now scoped to tracked files (`cad70d9`). That exposed a real one
underneath: `tools/designprobe` emitted a redundant element type that gofumpt strips, so its
generated file could never pass the gate it was written to pass (`824abd4`).

### What the sweeps confirmed clean

Every remaining difference found across all five sweeps is D-011's declared class — upstream prints
an unhandled Python traceback, the port prints a panel at the same exit code. That covers the whole
override surface (`--locale.language klingon`, `--design.page.size`, `--rendercv_settings.date`,
`--cv.sections.a.0`, `--output-folder-name`), a top-level scalar or sequence document, and an empty
flow key. Exit codes agreed on **every** case in every sweep after the fixes above.
...
| 1 | Conformance harness (corpus, gengolden, helpers) | [001](001-conformance-harness/spec.md) | **green — promoted 2026-08-14 (pass 25); the blocker was stale.** The fixed-length, checkout-independent work root already existed (`internal/conformance/workroot`, `Root = /tmp/rendercv-go-conformance`, `CaseDir = Root/run/<name>`), landed by `0a2a2ff` with its goldens first in `b915f8d`. Both halves of the old item are covered: the path is checkout-independent, and the shared-`caseWorkDir` race is closed by a per-case `flock` rather than by uniqueness, with a `checkCanonical` guard rejecting a symlinked root. What no prior audit had done is the run that settles it — `err_unknown_theme` and `err_missing_file` pass from the main checkout **and** from an agent worktree, so parallel fan-out can self-verify after all. Only two goldens bake the root, both confirmed | 55 corpus cases, 0 FAIL, 0 SKIP |
...
| 6 | Design & themes (9) + the settings schema | [006](006-design-and-themes/spec.md) | **green — promoted 2026-08-14 (pass 25).** The field-order interleave stays fixed (`e25215a`, pinned by `838d01d`). The open finding — a `design:` block with no `theme:` key rendering at exit 0 where upstream raises `KeyError: 'theme'` and exits 1 — is **closed** (`7f1ab2f`). The scoped decision it was waiting on was made by the merge owner: exit-code parity is contract axis 2 and was being violated, while the traceback text is already D-011's declared class, so the port exits 1 with its own panel — the precedent D-012 states for the four constructor crashes. Four document shapes were affected (`{}`, a mapping with only unknown keys, a mapping with only nested design keys, and one mixing them); all four now match on exit code, and the message is pydantic's own `union_tag_not_found` text, read out of the vendored venv rather than invented. Recorded as D-021. All 9 themes render byte-identical `.typ`/`.md`/`.html` | n/a |
...
| 8 | Templater (pongo2 env, filters, markdown→typst, processors) | [008](008-templater/spec.md) | **green — promoted 2026-08-14 (pass 25).** Both of the verifier's old blockers stay closed (`d4dea64`, `370fc88`, `bef8ead`), and the two items this row carried as open are now closed too. The sixth, unrecorded `markdown_to_typst` divergence — an unterminated character reference like `&#35` escaped to `\#35` instead of terminated to `&#35;` — is fixed at the preprocessor, where CPython's `html.parser` re-emits the reference from its digits (`a49a343`, 33 fixture rows, 23 red first). **Reference-style links on the Typst path are implemented** (`e1d8582`, `bc7f74a`): the declined reason — "the line-at-a-time Typst pass never builds the link-definition map" — did not survive measurement, because `convertBlock` *is* the block-processor dispatch and the map builds at upstream's own priority 15. The porter also measured that `md` is a module-level `Markdown` which never calls `reset`, so definitions are **process-wide**, and reproduced that rather than inventing a per-call map. 77 further shapes ran as a scratch differential with 0 mismatches. `markdown_to_typst.json` is **681 rows, 0 duplicates**. What remains open is narrower than the row used to claim: a code span whose body contains a line break, and `BACKTICK_RE`'s `(?<!\\)` guard | n/a |
...
| 10 | wazero + WASI typst → PDF, then PNG | [010](010-typst-compilation/spec.md) | **green — verified 2026-08-14, first fresh-context audit.** The compiler, the fonts and `fontawesome` are vendored and embedded (D-007, corrected). 26 hand-run differentials (17 font families, 9 themes, photo, CJK, A4, 29- and 316-page documents) found the port's PDFs and PNGs **byte-identical** to the vendored Python's — stronger than axis 1 requires — and font provisioning (77 vendored files, `fontawesome`, the `rendercv` typst package) sha256-identical to upstream's sources. One blocker found and fixed same day: an output path outside the `.typ`'s own directory (`-pdf out2/cv.pdf`, `settings.render_command.pdf_path` elsewhere) aborted the whole render where upstream renders it; `typstc.Compile` now mounts a second guest root only when the output is not already under the input's (`6e341e5`). One blocker recorded as a divergence rather than fixed: the WASI sandbox cannot see the host's installed fonts, so an unconstrained `font_family` override or any glyph the vendored set lacks (emoji, the practical case) diverges — D-007 corrected and the gap documented (`471cf5c`). Two coverage holes are closed: no corpus case exercised `cv.photo` before, `photo_next_to_typst` is
added (`d7e3aab`, `ba9ddde`); and no case exercised a Typst compiler failure, `err_typst_compile_failure`
is added, registered unreachable under D-014 for its stderr traceback (`f5e1fe4`, `24a3b15`). All
four remaining minors from the same verify are closed too: the compiler-failure stdout shape is
fixed to leave the progress panel rather than a clean Error box (`7a8131b`); `AssertPDF` now checks
embedded fonts as spec §1.2 already required (`5550905`); `typstc.Compile`'s `ctx` now actually
aborts a canceled call (`d37dea5`); one bundled historical commit (`9d26f8f`) is left as recorded
process history rather than rewritten | 20 / 20 in the suite |
| 11 | Markdown + HTML renderers | [011](011-markdown-and-html/spec.md) | **green — promoted 2026-08-14 (pass 25).** Both `knownRemainder` classes are closed, and the recorded diagnosis of the first one was wrong in an instructive way: the block tag in a container needed **no** stash-and-restore preprocessing pass, because `handle_starttag` sets `inraw` (`htmlparser.py:215`) and **`handle_endtag` has no such branch** (`:230-255`) — so the fix is a decline in `blockLevelHTMLParser.Open`, gated on `commonMarkCloseTag` so the `#include <stdio.h>` class stays exactly as wide as it was (`fc77a0c`, `01b5d8d`). The multi-line link destination needed only that `getLink` be fed the block's whole text as upstream feeds it (`inlinepatterns.py:698,731`) rather than `PeekLine`, plus `indentedLineStart` for goldmark's per-line `TrimLeftSpace` (`65aceb1`, `4d066ff`). `html.json` is **1238 rows, 0 duplicates**, all regenerated under the vendored `markdown.markdown` with 0 mismatches. A 40-construct end-to-end sweep through a real CV field — emphasis, code spans, links, images, raw HTML, entities, lists, tables, escapes — is byte-identical on `.typ`, `.md` and `.html` for **40 of 40**. Three narrower classes stay open and are named in the code: a code span inside a link destination, a list interrupting a paragraph, and `</ div>` | n/a |
| 12 | CLI (`new`, `render`, `create-theme`, overrides, watcher) | [012](012-cli/spec.md) | **green — re-promoted 2026-08-14 (pass 25), and the 2026-08-12 demotion's two blockers were both measured stale.** (a) The bad-indentation document that demotion cites now gives `main_yaml_file: line 4 to line 6` / `while parsing a block mapping.`, **byte-identical to upstream on both streams**. (b) "The port emits no ANSI colour at all on a tty" is false: on a pty the validation panel carries 84 escape sequences, and the final screen state is identical to upstream's once the nondeterministic 8-column timing field is masked. `TestProgressPanelColour` already compares that surface byte-for-byte across seven terminal configurations, and `TestValidationTableColour`, `TestErrorPanelColour`, `TestNewCommandColour`, `TestCreateThemeColour`, `TestTerminalDetection` and `TestDumbTerminalWidth` cover the rest. The one genuine stream difference is Rich's intermediate `Live` frames: `refresh_per_second=4` makes their count wall-clock dependent, so no byte-comparable artifact exists and the final screen — what a reader actually sees — is the assertion. A full CLI sweep (26 shapes: bad flags, unknown commands, missing files, overrides, `new`, `create-theme`, `--help`, `--version`) agreed on **every exit code**, with residual stdout differences only where upstream tracebacks (D-011) or where D-009's longer binary name re-pads a box | n/a |
| 13 | Parity closeout (sample generator, version, error handler, watcher, packaging) | [013](013-parity-closeout/spec.md) | **green — re-audited 2026-08-14 (pass 26); the "6 still open" carried since 2026-08-11 is stale, all six now measured resolved, none by this iteration's own work.** (major) The 9-of-13 `go vet` intermediate-commit failure is unbisectable, unrewritable history — left as is, same category as `9d26f8f`'s bundling in row 10, not something a re-audit can close. (major+minor) `tools/sampleprobe` staying undeleted was never the real defect — the fix the row itself named, "a live 198-case differential that does not depend on the tool's own captured fixture," **already exists**: `internal/cli/sample/upstream_conformance_test.go`'s `TestGenerateMatrix`, gated `//go:build conformance`, runs all 9×22 pairs against a freshly-started upstream interpreter and passes (6.9s). `sampleprobe` itself is legitimate as a data-*generation* tool, the same role `gengolden`/`gentemplates`/`localeprobe` play — nothing here calls for deleting it. (minor) The OS-Error slack is not unwritten: `specs/divergences.md` D-015 (approved, iteration 13, proposal P-3) documents exactly this, and `just test-parity`'s differential gate asserts it by name every run. (minor) `panic` stays banned in package `cli` — zero non-test occurrences, confirmed by grep. (minor) `err_unknown_theme` baking an absolute path is iteration 1's fixed-length work root (`workroot.Root`, `/tmp/rendercv-go-conformance`) doing exactly what it was built for — the same machine-independent path on every checkout, which is why the golden can bake it and still match everywhere | 42 / 42 `TestParity`, 3468 verbose PASS / 0 FAIL / 1 SKIP under the conformance tag, 7 / 7 behavior-31 differential rows |
| 14 | Lua-scripted custom themes (D-002) + the two folder messages | [014](014-lua-custom-themes/spec.md) | **green — promoted 2026-08-14 (pass 25); nothing was broken and the gate it waited on is gone.** D-002's two false claims are corrected against fresh measurement (`e4a9d53`): an unknown key on a scripted custom theme is exit 1/1 with **byte-identical** stdout (1411 B, `cmp` clean), and the `!!binary` claim was misfiled — it belongs to D-012, whose summary sentence was *inverted* (the port is stricter there, not laxer: upstream renders `!!binary aGk=` as `hi` at exit 0, the port exits 1). D-012's text is corrected too. Measured end to end this pass: `create-theme mytheme` then `render` produces **byte-identical `.typ`, `.md` and `.html`**, and the only difference anywhere in the flow is the one line naming `init.lua` instead of `__init__.py` — which is D-002 itself. Ten theme-name shapes agree on exit code | n/a |
| 16 | The public Go API (`pkg/rendercv`) | [016](016-public-api/spec.md) | **green — promoted 2026-08-14 (pass 25).** The 2026-08-12 verification's blocker and all four majors and two minors were fixed and re-measured then (`609300c`, `f66a582`, `5e33dfa`, `b65be86`); what stood were two *process* findings, which are history and not defects. Nothing this pass's sweeps found touches the library surface, and the verifier's end-to-end check still holds: an `engineeringresumes` + German-locale document rendered through the library, the CLI and the vendored Python is byte-identical across all three on `.typ`, `.md` and `.html` | n/a |
| 15 | Explicit YAML tags | [015](015-yaml-tags/spec.md) | **green — promoted 2026-08-14 (pass 25).** `internal/kindguard`'s two blind spots stay closed and re-proven red on planted violations, with 0 false positives. The three items this row listed as open are all closed: `repr(TaggedScalar)` at top level and inside a container (unit A), a non-string mapping **key** (unit B), and tagged keys (unit C, which cost no code — `KeyNode = buildNode(mv.Key)` made §4.1 fall out of A + B, exactly as the delta predicted). `pythonElemRepr` no longer exists; `schemaerr.PythonText` is the single renderer, so the theme and locale paths cannot drift. **All seven of spec-delta §7's acceptance criteria are met**, including the end-to-end one: a CV whose `design.theme` is `!!str classic` gives a byte-identical 1599-byte panel from the port and the vendored CLI. The delta's remaining unit D — container mapping keys — is **not implementation work**: goccy refuses those documents at parse, and re-measurement on five shapes shows upstream loads the key then raises, so **both sides exit 1 on all five** and the difference is D-011's message shape rather than a lost document. Declared under D-017, whose sequence-key row is corrected accordingly | n/a |
