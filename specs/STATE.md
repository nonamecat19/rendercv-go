...
| 12 | CLI (`new`, `render`, `create-theme`, overrides, watcher) | [012](012-cli/spec.md) | **green — promoted 2026-08-12 (pass 24).** Every recorded obstacle was re-measured and none survives. The blocker (click's `ignore_unknown_options`) **does not reproduce**: `render --nope`, `--version`, `-x`, `--helpx`, `--nokey=value` and `--typ out.typ CV.yaml` are exit 1/1 with the unknown token taken as the input file, gated at `internal/cli/args_test.go:215-245`. N1 does not reproduce (exit 2/2). `AssertUnreachable` now rebinds the binary name and compares artifact contents, so `new_typst_templates` is no longer held up by D-009. The last ungated defect — bad indentation reporting the wrong line and leaking a goccy coordinate — **was fixed this pass** (152 inputs enumerated, 115 mislocated before, 115/115 exact in message, line and column after). A **behavioral `--watch` divergence** found and fixed this pass: upstream's watchdog swallows the `EACCES` and watches on in silence where the port exited 1 having rendered nothing. Residual differences are Python tracebacks only, i.e. D-011's class, which is human-gated | 42 / 42 `TestParity` (8 inverted), 0 SKIP |
