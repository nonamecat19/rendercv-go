...
| 15 | Explicit YAML tags | [015](015-yaml-tags/spec.md) | **implemented, not yet verified by a fresh context.** The reader had no case for a tag node at all, so every tagged node became `KindNull`. 53 of a 63-case differential matrix are now byte-identical, up from 1; all 10 remaining divergences fall in a recorded class (D-012's three groups, D-011's internal errors, or the numeric-repr gap deferred since iteration 14's pass 13). Three defects the work *exposed* and that predate it are fixed here too, because 015's acceptance criteria could not pass around them | 53 / 63 differential, `TestParity` unchanged at 34/42 |
