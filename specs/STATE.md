...
| 4 | The DOI-URL-too-long record carries an empty schema location and is **dropped**: the user is told a section has problems and never which field. | **open, and narrower than it looks.** Entry errors normally reach stderr fine — the pipeline splices a wrapper's `Children` into the top-level list, and `cv.sections.e.0.start_date` prints correctly. Only the **empty-location** child is lost, because the splice has no path to prepend to. Making `cli.fail` recurse into `Children` surfaces it but **double-prints every other entry error**; that was tried and reverted. The fix belongs in the splice, not the renderer — **and a second look says the splice already handles it**: an empty-location child keeps the wrapper's own location, which would print. So the record is lost somewhere between `ValidatePublicationEntry` appending it and `spliceChildren` receiving it. **Traced to the end, and it is a genuine conflict rather than a bug to patch.** The chain: `ValidatePublicationEntry` emits the record with an **empty** location (spec §3.12 says that is verified upstream, and `TestPublicationDOIURLTooLong` asserts it); `spliceChildren` therefore gives it the *wrapper's* location, one element short of the entry's; and `dedup` — which keys on location alone — drops it as a duplicate of the wrapper.

Three fixes were tried and all three are wrong: recursing in `cli.fail` double-prints every other entry error; keying `dedup` on location **and** message breaks upstream's `photo` case, where two records at `cv.photo` with *different* messages must collapse to one; and giving the record the entry's location contradicts the spec-verified test.

**The likely resolution** is that both readings of "empty location" are true — pydantic's raw `loc` is empty, and pydantic then prepends the model's own path, so the *rendered* row is `cv.sections.<key>.<index>`. If so the port should keep the empty raw location and have the splice append the entry index, and `TestPublicationDOIURLTooLong` is testing the pre-prepend value. That needs confirming against upstream's raw records before anyone changes it. |
