...
1. **Iteration 8** — five `markdown_to_typst` divergences (raw inline HTML, a doubled backtick, an
   autolink, a mail autolink, an image producing a spurious visible link) plus a sixth found in this
   pass (`convertAdmonition` parses a `summary` line-by-line instead of over the joined block, so
   bold spanning a line break inside a summary never pairs). **A porter is fixing these now** (see
   below); this row will be updated when it lands.
2. **Iteration 6** — the missing-`theme`-key crash mismatch, recorded above. Needs a scoped decision
   on how the port represents an unhandled-exception-shaped upstream failure; not built yet.
3. **Iteration 11** — two of the three previously-recorded HTML divergences are still open and
   reachable end-to-end (`*a **bold** thing*` emphasis nesting, `[t](a b)` an unbracketed space in a
   link destination); the third (a raw block tag in a list item) was not re-probed this pass. Fixing
   either needs replacing part of goldmark's default inline-parser set with a hand-written parser
   mirroring python-markdown's regex tree processors — comparable in scope to the `markdown_to_typst`
   transform this port already has, not a quick fix. No spec task covers this yet.
4. **Iteration 1** — goldens still bake this machine's absolute path (`testdata/golden/err_unknown_theme/stdout.txt`
   and two others). Needs a golden regeneration, which is human-gated.
5. **Iteration 14** — two minor inverted-behavior cases: a control character in an Input Value
   renders raw where Rich strips it, and an Arabic-Indic-digit colour element is accepted upstream
   (exit 0) and rejected here (exit 1).
6. **Iteration 3** — the constructed-entry half of the discrimination criterion (spec F5) is still
   explicitly untested (`sectionvalidation_test.go:344-356`).
...
**Also found and fixed in this pass, not a code defect:** two ledger-integrity gaps.
`e25215a` (the iteration-6 interleave fix) landed with no test — closed by `838d01d`. And rows 12–14
of this table reference "the re-verification section below" / "listed below", which `308b6f4`
deleted when it trimmed this file — those sub-findings (iteration 13's "2 majors and 4 minors stay
open" in particular) are recorded nowhere now and would need reconstructing from git history if
they matter, or should simply be treated as closed if nobody can reproduce them.
